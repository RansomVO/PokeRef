      ...
      <body>
        ...
        <table border="1" class="KEY_TABLE">
          ...
          <tr>
            <td valign="top">
              <table>
                <!--<tr style="display:none;" comment="TODO QZX: Remove the 'style' attribute (and this 'comment' attribute) when sorting is supported.">-->
                <tr>
                  <td>Sort By:</td>
                  <td >
      ...
      <tr align="left" style="border-top-width:5px">
        ...
        <xsl:attribute name="fastMoves">
          <xsl:call-template name="MovesAttribute">
            <xsl:with-param name="Moves" select="../MoveSet[Pokemon/ID=$PokemonID]/FastAttack/Move" />
          </xsl:call-template>
        </xsl:attribute>
        <xsl:attribute name="chargedMoves">
          <xsl:call-template name="MovesAttribute">
            <xsl:with-param name="Moves" select="../MoveSet[Pokemon/ID=$PokemonID]/ChargedAttack/Move" />
          </xsl:call-template>
        </xsl:attribute>
