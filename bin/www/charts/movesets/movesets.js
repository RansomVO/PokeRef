...
function FilterTable(table) {
    // Iterate throw all of the Pokémon in the table.
...
// Filter the Pokémon and its MoveSets and hide what doesn't match the filter.
function FilterPokemon(pokemonRows) {
    // See if this Pokémon should be filtered out.
    var hidePokémon = !PokemonMatchesFilter(pokemonRows[0]);

    // Iterate through all of the MoveSets and show/hide the rows appropriately.
    var movesetsFound = 0;
    for (var i = pokemonRows.length - 1; i > 0; i--) {
        if (hidePokémon || !MoveSetMatchesFilter(pokemonRows[i])) {
...
// Determine whether the Pokémon in this row should be shown or not.
